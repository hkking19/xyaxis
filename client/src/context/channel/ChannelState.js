...
import {  SET_CHANNELS,SET_CHANNEL, SOCKET_CONNECTION, PUBLIC_CHANNELS,SET_SEARCHED_CHANNELS,SET_SEARCHING,REMOVE_SEARCHED_CHANNELS } from '../type';
...
const ChannelState = (props) => {
    ...
    const getAllPublicChannels = async () => {
        ...
        if(token) {
            setAuthToken(token)
            const res = await axios.get('http://localhost:3001/api/channel/getPublicRooms?public=true');
            const data = res.data;
            if (data.error) {
    ...
    const createChannel = async (channel) => {
        ...
        if(token) {
            ...
            try {
                ...
                console.log(channel)
                const res = await axios.post('http://localhost:3001/api/channel/create',channel);
                const data = res.data;
                console.log(data)
            } catch (error) {
                if(error.response) {
    ...
    const joinChannel = async (channelData) => {
        ...
        if(token) {
            ...
            try {
                ...
                const res = await axios.post('http://localhost:3001/api/channel/join',channelData);
            } catch (error) {
                if(error.response) {
                    const msg = {
                        message: error.response.data.error,
    ...
    const getAllUsersChannels = async () => {
        ...
        if(token) {
            setAuthToken(token)
            try {
                const res = await axios.get('http://localhost:3001/api/channel');
                if (res.data) {
    ...
    const searchChannel = async (key) => {
        ...
        if(token) {
            ...
            try {
                setSearching(true)
                const res = await axios.get(`http://localhost:3001/api/channel/find?search=${key}`);
                if(key !== '' && res.data.length === 0) {
                    dispatch({ type: SET_SEARCHED_CHANNELS, payload: 'No Data Found' });
                }
                else if (res.data) {
                    removeSearchChannels()
                    dispatch({ type: SET_SEARCHED_CHANNELS, payload: res.data });
                }
            } catch (error) {
                if(error.response) {
                    const msg = {
